...
@section('title', 'Edit Contract - Car Empire Management System')

@section('content')
@php
    $linkedTo = old('linked_to', $contract->vehicle_id ? 'vehicle' : ($contract->employee_id ? 'employee' : 'vehicle'));
    $vehicleDisplay = $contract->vehicle_id && $contract->vehicle ? ($contract->vehicle->plate_number ? $contract->vehicle->plate_number . ' — ' : '') . $contract->vehicle->full_name : '';
@endphp
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">
            <i class="fas fa-file-contract me-2"></i>Edit Contract
        </h1>
            ...
            <form action="{{ route('contracts.update', $contract) }}" method="POST" enctype="multipart/form-data" id="contractForm">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="linked_to" id="linked_vehicle" value="vehicle" {{ $linkedTo === 'vehicle' ? 'checked' : '' }}>
                                <label class="form-check-label" for="linked_vehicle">Vehicle</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="linked_to" id="linked_employee" value="employee" {{ $linkedTo === 'employee' ? 'checked' : '' }}>
                    ...
                    <div class="col-12" id="vehicle_block">
                        ...
                        <input type="text" class="form-control @error('vehicle_id') is-invalid @enderror" id="vehicle_search" placeholder="Type plate number, make, model or year..." autocomplete="off" value="{{ old('vehicle_display', $vehicleDisplay) }}">
                        <input type="hidden" name="vehicle_id" id="vehicle_id" value="{{ old('vehicle_id', $contract->vehicle_id) }}">
                    ...
                    <div class="col-12" id="employee_block" style="display: none;">
                        ...
                        <select class="form-select @error('employee_id') is-invalid @enderror" id="employee_id" name="employee_id">
                            ...
                            @foreach($employees as $emp)
                                <option value="{{ $emp->id }}" {{ old('employee_id', $contract->employee_id) == $emp->id ? 'selected' : '' }}>{{ $emp->full_name }}</option>
                            @endforeach
                        </select>
                        @error('employee_id')
                    ...
                    <div class="col-md-8">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $contract->title) }}" required placeholder="e.g. Employment Contract - Juan Dela Cruz">
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('contract_type') is-invalid @enderror" id="contract_type" name="contract_type" required>
                            @foreach(\App\Models\Contract::typeOptions() as $opt)
                                <option value="{{ $opt }}" {{ old('contract_type', $contract->contract_type) === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                        @error('contract_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('party_name') is-invalid @enderror" id="party_name" name="party_name" value="{{ old('party_name', $contract->party_name) }}" placeholder="Name or company">
                        @error('party_name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="date" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date" value="{{ old('start_date', $contract->start_date?->format('Y-m-d')) }}">
                        @error('start_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="date" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date" value="{{ old('end_date', $contract->end_date?->format('Y-m-d')) }}">
                        @error('end_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="2" placeholder="Brief description">{{ old('description', $contract->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        @if($contract->has_file)
                            <small class="text-muted">Current: <a href="{{ $contract->file_url }}" target="_blank">View file</a>. Upload a new file to replace.</small>
                        @else
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            @foreach(\App\Models\Contract::statusOptions() as $opt)
                                <option value="{{ $opt }}" {{ old('status', $contract->status) === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2" placeholder="Optional">{{ old('notes', $contract->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
