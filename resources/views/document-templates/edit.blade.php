...
@section('title', 'Edit Form Template - Car Empire Management System')
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('document-templates.update', $template) }}" id="templateForm">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select @error('document_type') is-invalid @enderror" 
                                            ...
                                            @foreach($documentTypes as $key => $label)
                                                <option value="{{ $key }}" {{ old('document_type', $template->document_type) == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const existingFields = @json($template->form_fields ?? []);
    ...
    function addField(fieldData = {}) {
        ...
        if (fieldData.options) {
            if (Array.isArray(fieldData.options)) {
                optionsValue = fieldData.options.join(', ');
            } else {
                optionsValue = fieldData.options;
            }
        }
    ...
    if (existingFields.length > 0) {
        ...
        // Add first field by default if no existing fields
        addField();
    }
