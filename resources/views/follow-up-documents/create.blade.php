...
@section('title', 'Add Follow Up Document - Car Empire Management System')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">
            <i class="fas fa-clipboard-check me-2"></i>Add Follow Up Document
        </h1>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('follow-up-documents.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-8">
                        <label for="title" class="form-label">Title <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" required placeholder="e.g. OR/CR for ABC 1234">
                        @error('title')
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('priority') is-invalid @enderror" id="priority" name="priority">
                            ...
                            @foreach(\App\Models\FollowUpDocument::priorityOptions() as $opt)
                                <option value="{{ $opt }}" {{ old('priority') === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                        @error('priority')
                            <div class="invalid-feedback">{{ $message }}</div>
