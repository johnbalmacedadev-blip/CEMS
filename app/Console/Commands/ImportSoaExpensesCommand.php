...
class ImportSoaExpensesCommand extends Command
{
    protected $signature = 'import:soa-expenses
                            {--year=2026 : Calendar year to import/clear}
                            {--file= : Path to JSON export}
                            {--fresh : Delete existing SOA data for that year (Flagship/Warehouse/Annex) before import}
                            {--dry-run : Parse and report without writing}';

    protected $description = 'Import EXPENSES workbook JSON into SOA (daily budgets, cash additions, manual debit lines)';
    ...
    public function handle(): int
    {
        ...
        $path = $this->option('file') ?: storage_path("app/soa_expenses_{$year}_import.json");
        if (! is_file($path)) {
        ...
        $this->info("Year {$year} | Days: ".count($days)." | Expense lines: {$expenseLines}");
        ...
        if ($this->option('fresh')) {
            $this->clearYearSoaData(array_values($pmIds), $year);
        }
        ...
        foreach ($days as $day) {
            try {
                DB::transaction(function () use ($day, $pmIds, $year, &$budgets, &$cashAdds, &$debits) {
                    ...
                    DailyBudget::updateOrCreate(
                        ...
                        [
                            ...
                            'added_cash' => $added,
                            'notes' => "Imported from EXPENSES {$year} (".($day['sheet'] ?? $tier).')',
                        ]
                    );
                    $budgets++;
     ...
     */
    private function clearYearSoaData(array $paymentMethodIds, int $year): void
    {
        $this->warn("Clearing {$year} SOA data for Flagship / Warehouse / Annex...");

        DailyBudget::whereIn('payment_method_id', $paymentMethodIds)
            ->whereYear('budget_date', $year)
            ->delete();
        ...
        $this->info("Cleared existing {$year} SOA rows for those cost centers.");
    }
}
